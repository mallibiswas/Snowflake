...
create or replace procedure ZENSTAG.AMS.CONTRACT_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSTAG.AMS.CONTRACT as
          select
            $1 as id,
          ...
          FROM @ZENSTAG.AMS.AMS_S3_STAGE/${FILE_DATE}/contract.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSTAG.AMS.CONTRACT_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENSTAG.AMS.CONTRACT_PROCEDURE(CURRENT_DATE());

alter task ZENSTAG.AMS.CONTRACT_TASK resume;