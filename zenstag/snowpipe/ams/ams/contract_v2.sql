...
create or replace procedure ZENSTAG.AMS.CONTRACT_V2_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSTAG.AMS.CONTRACT_V2 as
          select
            $1 as id,
          ...
          FROM @ZENSTAG.AMS.AMS_S3_STAGE/${FILE_DATE}/contract_v2.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSTAG.AMS.CONTRACT_V2_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENSTAG.AMS.CONTRACT_V2_PROCEDURE(CURRENT_DATE());

alter task ZENSTAG.AMS.CONTRACT_V2_TASK resume;