...
-- pause pipe
alter pipe ZENSTAG.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_SNOWPIPE set pipe_execution_paused = true;
-- ensure pending files is 0
select system$pipe_status('ZENSTAG.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_SNOWPIPE'); 
...
COPY INTO ZENSTAG.PRESENCE.WIFI_ENRICHED_SIGHTINGS
FROM (SELECT 
...
drop table if exists ZENSTAG.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS

-- Update ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE

-- pause pipe
alter pipe ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE set pipe_execution_paused = true;
-- ensure pending files is 0
select system$pipe_status('ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE'); 
-- re-create pipe to copy into the old table
create or replace pipe ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSTAG.PRESENCE.WIFI_UNKNOWNAP_BLIP
FROM (SELECT 
    nvl($1:ap_type, 'APType_UNKNOWN')::string as ap_type,
    ZENSTAG.PRESENCE.string_to_mac(HEX_ENCODE(BASE64_DECODE_BINARY($1:ap_mac::string))) as ap_mac,

    ZENSTAG.PRESENCE.string_to_mac(HEX_ENCODE(BASE64_DECODE_BINARY($1:client_mac::string))) as client_mac,
...
FROM @ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_S3_STAGE)
on_error = 'continue';
-- ensure pipe execution state is running
select system$pipe_status('ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE'); 

-- remove unnecessary new table
drop table if exists ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS
