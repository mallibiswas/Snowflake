...
create stage if not exists ZENSTAG.PRESENCE.ARCHIVER_LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_S3_STAGE
    file_format = ZENSTAG.PRESENCE.PRESENCE_CSV_FORMAT
    url = 's3://zs-uw2-data-archives/rds/wifisightingsclassifierconfig/location_classifier_config_thresholds'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-rds-location-classifier-config-thresholds-snowflake-stage');

create or replace TABLE ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS (
...
create or replace TABLE ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE (
...
create pipe if not exists ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE (
	LOCATION_ID,
...
FROM @ZENSTAG.PRESENCE.ARCHIVER_LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_S3_STAGE
on_error = 'continue';

create stream if not exists ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES on table ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE;

create task if not exists ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_UPSERT_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES')
AS
    MERGE INTO ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS as lcgt USING (
        SELECT
            location_id,
            min_walkin_dwell_time_seconds,
            max_walkin_dwell_time_seconds,
        ...
        FROM ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES
...
ALTER TASK ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_UPSERT_TASK RESUME;
