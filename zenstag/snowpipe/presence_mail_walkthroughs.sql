...
CREATE OR REPLACE STAGE ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zs-uw2-platform-kafka-archives/archiver/mail_email_walkthroughs_bymessagelogid/'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-archiver-mail-walkthroughs-snowflake-stage');


CREATE OR REPLACE TABLE ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS(
...
CREATE OR REPLACE TABLE ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE(
...
CREATE PIPE IF NOT EXISTS ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE(RAW_RECORD)
FROM @ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_S3_STAGE
on_error = 'continue';
...
CREATE STREAM IF NOT EXISTS ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE_CHANGES ON TABLE ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE;
...
create task if not exists ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_UPSERT_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE_CHANGES')
AS
    MERGE INTO ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS as wt USING (
        SELECT
        ...
        FROM ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE_CHANGES QUALIFY ROW_NUMBER() OVER (PARTITION BY messagelog_id order by INSERT_ID desc) = 1
...
ALTER TASK ZENSTAG.PRESENCE.MAIL_WALKTHROUGHS_UPSERT_TASK RESUME;
