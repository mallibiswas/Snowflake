create stage if not exists ZENSTAG.PRESENCE.ARCHIVER_WIFI_CONSENTED_SIGHTINGS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zs-uw2-platform-kafka-archives/archiver/presence_wifi_consentedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-archiver-wifi-consentedsighting-snowflake-stage');

create table if not exists ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS (
...
create table if not exists ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE (
...
create pipe if not exists ZENSTAG.PRESENCE.ARCHIVER_WIFI_CONSENTED_SIGHTINGS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE(RAW_RECORD)
FROM @ZENSTAG.PRESENCE.ARCHIVER_WIFI_CONSENTED_SIGHTINGS_S3_STAGE
on_error = 'continue';
...
create stream if not exists ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE_CHANGES on table ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE;
...
create or replace task ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_UPSERT_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE_CHANGES')
AS
    MERGE INTO ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS as wfs USING (
        SELECT
            ...
            ZENSTAG.PRESENCE.DECODE_CLIENT_MAC_INFO($1:client_mac_info::array) as anonymous_client_mac_info,
        ...
        FROM ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE_CHANGES QUALIFY ROW_NUMBER() OVER (PARTITION BY sighting_id order by INSERT_ID desc) = 1
...
ALTER TASK ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS_UPSERT_TASK RESUME;
