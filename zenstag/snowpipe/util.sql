...
create function if not exists ZENSTAG.PRESENCE.string_to_mac(A string)
    returns string
    language javascript
  as
$$
...
create or replace function ZENSTAG.PRESENCE.decode_client_mac_info(A array)
    returns array
    language javascript
  as
$$
...
create or replace file format ZENSTAG.PRESENCE.S3_PARQUET_FORMAT
  type = 'PARQUET'
