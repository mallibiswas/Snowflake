...
create or replace procedure ZENSTAG.HOTSPOT2.DEVICE_PROFILE_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSTAG.HOTSPOT2.DEVICE_PROFILE as
          select
          ...
          FROM @ZENSTAG.HOTSPOT2.HOTSPOT2_S3_STAGE/${FILE_DATE}/device_profile.csv;`
     }).execute();
$$;


-- Create task to call the procedure (need to avoid overlapping the S3 archiver, S3 is 0, 12 hour)
create or replace task ZENSTAG.HOTSPOT2.DEVICE_PROFILE_TASK
    WAREHOUSE = ZENSTAG
...
as 
    ...
    CALL ZENSTAG.HOTSPOT2.DEVICE_PROFILE_PROCEDURE(TO_DATE(CONVERT_TIMEZONE('UTC', CURRENT_TIMESTAMP())));

alter task ZENSTAG.HOTSPOT2.DEVICE_PROFILE_TASK resume;
