...
create or replace procedure ZENSTAG.POS.CUSTOMER_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSTAG.POS.CUSTOMER as	
          select	
              $1::string as id,
          ...
          FROM @ZENSTAG.POS.ARCHIVER_POS_S3_STAGE/customer/${FILE_DATE}.csv;`
     }).execute();
$$;

-- Create task to call the procedure
create task ZENSTAG.POS.CUSTOMER_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSTAG.POS.CUSTOMER_PROCEDURE(CURRENT_DATE());

alter task ZENSTAG.POS.CUSTOMER_TASK resume;
