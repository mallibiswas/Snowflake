...
create OR replace stage ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zs-uw2-platform-kafka-archives/archiver/presence_wifi_unknownap_blip_byclientmac/'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-archiver-wifi-unknownap-blips-snowflake-stage');

-- Table will hold archiver data from our snowpipe
create table if not exists ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS (
...
create pipe if not exists ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE auto_ingest = true as 
COPY INTO ZENSTAG.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS
FROM (SELECT 
