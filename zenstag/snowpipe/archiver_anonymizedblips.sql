...
create OR replace stage ZENSTAG.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zs-uw2-platform-kafka-archives/archiver/anonymizedblips_anonymizedblip_byclientmac_0/'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-archiver-anonymizedblips-snowflake-stage');

-- Table will hold archiver data from our snowpipe
create table if not exists ZENSTAG.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS (
...
create pipe if not exists ZENSTAG.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS_SNOWPIPE auto_ingest = true as 
COPY INTO ZENSTAG.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS
FROM (SELECT 
...
FROM @ZENSTAG.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS_S3_STAGE)
on_error = 'continue';
