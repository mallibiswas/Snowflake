...
create OR replace stage ZENSTAG.PRESENCE.WIFI_ENRICHED_SIGHTINGS_S3_STAGE
    file_format = ( format_name = 'ZENSTAG.PRESENCE.S3_PARQUET_FORMAT' )
    url = 's3://zs-uw2-platform-kafka-archives/secor/presence_wifi_enrichedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-wifi-enrichedsighting-snowflake-stage');

create or replace table ZENSTAG.PRESENCE.WIFI_ENRICHED_SIGHTINGS (
...
create or replace pipe ZENSTAG.PRESENCE.WIFI_ENRICHED_SIGHTINGS_SNOWPIPE auto_ingest=true as
...
FROM @ZENSTAG.PRESENCE.WIFI_ENRICHED_SIGHTINGS_S3_STAGE)
on_error = 'continue';
