...
create or replace procedure ZENSTAG.CRM.PORTAL_ROUTER_TYPE_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSTAG.CRM.PORTAL_ROUTER_TYPE as
    select  
      $1:_id::string as id,
    ...
    FROM @ZENSTAG.CRM.MONGO_S3_STAGE/${FILE_DATE}/portal_routertype.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSTAG.CRM.PORTAL_ROUTER_TYPE_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSTAG.CRM.PORTAL_ROUTER_TYPE_PROCEDURE(CURRENT_DATE());

alter task ZENSTAG.CRM.PORTAL_ROUTER_TYPE_TASK resume;
