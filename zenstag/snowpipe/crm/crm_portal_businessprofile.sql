...
create stage if not exists ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zs-uw2-data-archives/mongo/portal_businessprofile/'
    credentials = ( aws_role = 'arn:aws:iam::255794285552:role/zs-uw2-mongo-snowflake-stage');

-- Table will hold raw mongo data from our snowpipe
create table if not exists ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_ARCHIVE (
...
create pipe if not exists ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_SNOWPIPE auto_ingest=true as
COPY INTO ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_ARCHIVE(RAW_RECORD)
FROM @ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_S3_STAGE
on_error = 'continue';
...
create stream if not exists ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_STREAM on table ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_ARCHIVE;
...
create task if not exists ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_UPSERT_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_STREAM')
AS
    MERGE INTO ZENSTAG.CRM.PORTAL_BUSINESSPROFILE as pbp USING (
        SELECT
        ...
        FROM ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_STREAM
...
ALTER TASK ZENSTAG.CRM.MONGO_PORTAL_BUSINESSPROFILE_UPSERT_TASK RESUME;
