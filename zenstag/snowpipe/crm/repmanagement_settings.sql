...
create or replace procedure ZENSTAG.CRM.REPMANAGEMENT_SETTINGS_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSTAG.CRM.REPMANAGEMENT_SETTINGS as
    select  
    ...
    FROM @ZENSTAG.CRM.MONGO_S3_STAGE/${FILE_DATE}/repmanagement_settings.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSTAG.CRM.REPMANAGEMENT_SETTINGS_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSTAG.CRM.REPMANAGEMENT_SETTINGS_PROCEDURE(CURRENT_DATE());

alter task ZENSTAG.CRM.REPMANAGEMENT_SETTINGS_TASK resume;
