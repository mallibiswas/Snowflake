...
create or replace procedure ZENSTAG.CRM.SMBSITE_IMPORTLOG_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSTAG.CRM.SMBSITE_IMPORTLOG as
    select  
    ...
    FROM @ZENSTAG.CRM.MONGO_S3_STAGE/${FILE_DATE}/smbsite_importlog.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute) 
create task ZENSTAG.CRM.SMBSITE_IMPORTLOG_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSTAG.CRM.SMBSITE_IMPORTLOG_PROCEDURE(CURRENT_DATE());

alter task ZENSTAG.CRM.SMBSITE_IMPORTLOG_TASK resume;
