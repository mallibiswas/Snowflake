CREATE OR REPLACE VIEW ZENSTAG.ADS.CAMPAIGN_WALKTHROUGHS COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for camapaign-level location walkthroughs reported daily'
AS
...
, walkthroughs_until_2019_11_15_ as (
  ...
  left join zenstag.ads.insights i 
  ...
  left join zenstag.ads.campaigns c on cd.campaign_id = c.campaign_id
...
, walkthroughs_after_2019_11_15_ as (
  ...
  left join zenstag.ads.campaigns c on w.campaign_id = c.campaign_id
  group by 1,2,3,4,5,6,7,8
)
...
, custom_conversion_walkthroughs_ as (
  ...
  left join zenstag.ads.sample_rates s on cccd.business_id = s.business_id and cccd.date = s.day 
  group by 1,2,3,4,5
)
...
, campaign_walkthroughs_ as (
  ...
  left join zenstag.ads.campaigns c on ccw.campaign_id = c.campaign_id
)
