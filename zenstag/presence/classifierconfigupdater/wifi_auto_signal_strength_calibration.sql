...
create table if not exists ZENSTAG.PRESENCE.WIFI_AUTO_SIGNAL_STRENGTH_CALIBRATION (
...
create or replace task ZENSTAG.PRESENCE.WIFI_CALCULATE_SIGNAL_STRENGTH_TASK
    WAREHOUSE = ZENSTAG
    SCHEDULE = 'USING CRON 0 0 * * 1 America/Los_Angeles' -- At 00:00 on Monday in PDT tz
AS
    MERGE INTO ZENSTAG.PRESENCE.WIFI_AUTO_SIGNAL_STRENGTH_CALIBRATION as calibration USING (
        WITH rssi_percentiles_ AS (
            ...
            FROM ZENSTAG.PRESENCE.WIFI_CONSENTED_SIGHTINGS
            WHERE
        ...
        , previous_config_ AS (
            ...
            FROM ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS l
            JOIN (
                SELECT
                    location_id
                    , MAX(effective_as_of) AS last_set
                FROM ZENSTAG.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS
                GROUP BY location_id
        ...
        , calibration_bundle_ AS (
            ...
            JOIN ZENSTAG.CRM.PORTAL_BUSINESSPROFILE bp ON s.location_id = bp.business_id
        )
...
ALTER TASK IF EXISTS  ZENSTAG.PRESENCE.WIFI_CALCULATE_SIGNAL_STRENGTH_TASK RESUME;
