-- These scripts were created to resolve an issue on Jun 25 2020 where sightings split due to lag in the blip section of the presence pipeline.
...
-- QUERY 1 - PULL OUT WIFI FINISHED SIGHTINGS
CREATE OR REPLACE TABLE zenprod.presence.wifi_finished_sightings_2020_06_25_omit
as
with deduped as (
select distinct *
FROM ZENPROD.PRESENCE.WIFI_FINISHED_SIGHTINGS AS s
WHERE s.end_time > '2020-06-25T16:00:00'
)
, sightings_ as (
  SELECT 
    DATEDIFF(
    ...
    , s.*
  FROM deduped AS s
  WHERE s.end_time > '2020-06-25T16:00:00' -- use end time slightly over an hour before incident started to catch all possible affected sightings
  AND s.end_time < '2020-06-26T01:00:00' -- cap at an end time so if we have to rerun we'll get the same results
...
CREATE OR REPLACE TABLE zenprod.presence.wifi_consented_sightings_2020_06_25_omit
as
with deduped as (
    select distinct *
...
select s.*
from deduped AS s
join zenprod.presence.wifi_finished_sightings_2020_06_25_omit as o on s.sighting_id = o.sighting_id



-- QUERY 3 - DELETE FROM WIFI FINISHED SIGHTINGS
DELETE FROM ZENPROD.PRESENCE.WIFI_FINISHED_SIGHTINGS s
USING zenprod.presence.wifi_finished_sightings_2020_06_25_omit o where s.sighting_id = o.sighting_id;



-- QUERY 4 - DELETE FROM WIFI CONSENTED SIGHTINGS
DELETE FROM ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS s
USING zenprod.presence.wifi_consented_sightings_2020_06_25_omit o where s.sighting_id = o.sighting_id;
