CREATE OR REPLACE VIEW ZENPROD.ADS.AD_SUMMARY COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for ad-level metrics such as impressions, clicks & spend reported daily'
AS

with facebook_ad_summary_ as (
    select
        parent_id
        , parent_name
        , ad_account_id
        ...
        , cpwt_28_day
    from zenprod.ads.facebook_ad_summary
)

, liveramp_ad_summary_ as (
    select
        parent_id
        , parent_name
        , ad_account_id
...
select * from facebook_ad_summary_ union all select * from liveramp_ad_summary_;
