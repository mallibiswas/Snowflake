CREATE OR REPLACE VIEW ZENPROD.ADS.FACEBOOK_AD_SET_WALKTHROUGHS COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for ad set level metrics such as impressions, clicks & spend reported daily'
AS
...
, ad_set_date_range_ as (
  ...
  from zenprod.ads.ad_set_insights i
...
, ad_set_walkthroughs_after_2019_11_15_ as (
  select 
    ...
    , case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(confirmed_walkthroughs * s.sample_rate_multiplier)
        when ad_set_goal = 'LOYALTY' then round(confirmed_walkthroughs * 1) -- no sampling required
        when ad_set_goal = 'LOST_CUSTOMER' then round(confirmed_walkthroughs * 1) -- no sampling required
        when ad_set_goal is null then round(confirmed_walkthroughs * s.sample_rate_multiplier) -- if unknown, default to sampling
    ...
    , case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(confirmed_walkthroughs_1_day * s.sample_rate_multiplier)
        when ad_set_goal = 'LOYALTY' then round(confirmed_walkthroughs_1_day * 1) -- no sampling required
        when ad_set_goal = 'LOST_CUSTOMER' then round(confirmed_walkthroughs_1_day * 1) -- no sampling required
        when ad_set_goal is null then round(confirmed_walkthroughs_1_day * s.sample_rate_multiplier) -- if unknown, default to sampling
    ...
    , case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(confirmed_walkthroughs_7_day * s.sample_rate_multiplier)
        when ad_set_goal = 'LOYALTY' then round(confirmed_walkthroughs_7_day * 1) -- no sampling required
        when ad_set_goal = 'LOST_CUSTOMER' then round(confirmed_walkthroughs_7_day * 1) -- no sampling required
        when ad_set_goal is null then round(confirmed_walkthroughs_7_day * s.sample_rate_multiplier) -- if unknown, default to sampling
    ...
    , case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(confirmed_walkthroughs_28_day * s.sample_rate_multiplier)
        when ad_set_goal = 'LOYALTY' then round(confirmed_walkthroughs_28_day * 1) -- no sampling required
        when ad_set_goal = 'LOST_CUSTOMER' then round(confirmed_walkthroughs_28_day * 1) -- no sampling required
        when ad_set_goal is null then round(confirmed_walkthroughs_28_day * s.sample_rate_multiplier) -- if unknown, default to sampling
  ...
  left join zenprod.ads.ad_set_insights i 
...
, custom_conversion_ad_set_insights_ as (
  ...
  from zenprod.ads.ad_set_insights_custom_conversions aiicc
  left join zenprod.ads.ad_sets a on aiicc.ad_set_id = a.ad_set_id
