...
create OR replace stage ZENPROD.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zp-uw2-foundation-kafka-archives/archiver/presence_wifi_enrichedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-archiver-wifi-enrichedsighting-snowflake-stage');

create or replace table ZENPROD.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS (
...
COPY INTO ZENPROD.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS
FROM (SELECT 
