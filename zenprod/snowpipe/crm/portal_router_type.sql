...
create or replace procedure ZENPROD.CRM.PORTAL_ROUTER_TYPE_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENPROD.CRM.PORTAL_ROUTER_TYPE as
    select  
      $1:_id::string as id,
    ...
    FROM @ZENPROD.CRM.MONGO_S3_STAGE/${FILE_DATE}/portal_routertype.json;`
     }).execute();
$$;


-- Create task to call the procedure 
create task ZENPROD.CRM.PORTAL_ROUTER_TYPE_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON 0 8 * * * UTC'
as 
    CALL ZENPROD.CRM.PORTAL_ROUTER_TYPE_PROCEDURE(DATEADD(day, -1, CURRENT_DATE()));

alter task ZENPROD.CRM.PORTAL_ROUTER_TYPE_TASK resume;
