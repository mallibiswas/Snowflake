...
create or replace procedure ZENPROD.CRM.SMBSITE_OFFERLOG_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENPROD.CRM.SMBSITE_OFFERLOG as
    select  
    ...
    FROM @ZENPROD.CRM.MONGO_S3_STAGE/${FILE_DATE}/smbsite_offerlog.json;`
     }).execute();
$$;


-- Create task to call the procedure
create task ZENPROD.CRM.SMBSITE_OFFERLOG_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON 0 8  * * * UTC'
as 
    CALL ZENPROD.CRM.SMBSITE_OFFERLOG_PROCEDURE(DATEADD(day, -1, CURRENT_DATE()));

alter task ZENPROD.CRM.SMBSITE_OFFERLOG_TASK resume;
