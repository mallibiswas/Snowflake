...
create stage if not exists ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zp-uw2-data-archives/mongo/portal_businessprofile/'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-mongo-snowflake-stage');

-- Table will hold raw mongo data from our snowpipe
create table if not exists ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_ARCHIVE (
...
create pipe if not exists ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_SNOWPIPE auto_ingest=true as
COPY INTO ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_ARCHIVE(RAW_RECORD)
FROM @ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_S3_STAGE
on_error = 'continue';
...
create stream if not exists ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_STREAM on table ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_ARCHIVE;
...
create task if not exists ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_UPSERT_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_STREAM')
AS
    MERGE INTO ZENPROD.CRM.PORTAL_BUSINESSPROFILE as pbp USING (
        SELECT
        ...
        FROM ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_STREAM
...
ALTER TASK ZENPROD.CRM.MONGO_PORTAL_BUSINESSPROFILE_UPSERT_TASK RESUME;
