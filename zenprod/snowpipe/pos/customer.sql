...
create or replace procedure ZENPROD.POS.CUSTOMER_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENPROD.POS.CUSTOMER as	
          select	
              $1::string as id,
          ...
          FROM @ZENPROD.POS.ARCHIVER_POS_S3_STAGE/customer/${FILE_DATE}.csv;`
     }).execute();
$$;

-- Create task to call the procedure
create task ZENPROD.POS.CUSTOMER_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON 0 * * * * UTC'
as 
    CALL ZENPROD.POS.CUSTOMER_PROCEDURE(CURRENT_DATE());

alter task ZENPROD.POS.CUSTOMER_TASK resume;
