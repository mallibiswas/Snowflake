...
create or replace procedure ZENPROD.POS.MERCHANT_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENPROD.POS.MERCHANT as	
          select
            $1::string as id,
          ...
          FROM @ZENPROD.POS.ARCHIVER_POS_S3_STAGE/merchant/${FILE_DATE}.csv;`
     }).execute();
$$;

-- Create task to call the procedure
create task ZENPROD.POS.MERCHANT_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON 0 * * * * UTC'
as 
    CALL ZENPROD.POS.MERCHANT_PROCEDURE(CURRENT_DATE());

alter task ZENPROD.POS.MERCHANT_TASK resume;
