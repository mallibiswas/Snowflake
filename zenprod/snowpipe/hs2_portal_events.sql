...
create OR replace stage  ZENPORD.PRESENCE.ARCHIVER_HS2_PORTALEVENTS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zp-uw2-foundation-kafka-archives/archiver/hs2_portalevents_byid/'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-archiver-hs2-portalevents-snowflake-stage');

create or replace table ZENPROD.PRESENCE.ARCHIVER_HS2_PORTALEVENTS (
    ID VARCHAR(16777216),
    TIMESTAMP TIMESTAMP,
...
create or replace pipe ZENPROD.PRESENCE.ARCHIVER_HS2_PORTALEVENTS_SNOWPIPE auto_ingest=true as
COPY INTO ZENPROD.PRESENCE.ARCHIVER_HS2_PORTALEVENTS
FROM (SELECT 
     ...
     FROM @ZENPROD.PRESENCE.ARCHIVER_HS2_PORTALEVENTS_S3_STAGE)
on_error = 'continue';
