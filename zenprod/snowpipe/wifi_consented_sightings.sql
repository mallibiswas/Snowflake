create stage if not exists ZENPROD.PRESENCE.ARCHIVER_WIFI_CONSENTED_SIGHTINGS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zp-uw2-foundation-kafka-archives/archiver/presence_wifi_consentedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-archiver-wifi-consentedsighting-snowflake-stage');

create table if not exists ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS (
...
create table if not exists ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE (
...
create pipe if not exists ZENPROD.PRESENCE.ARCHIVER_WIFI_CONSENTED_SIGHTINGS_SNOWPIPE auto_ingest=true as
COPY INTO ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE(RAW_RECORD)
FROM @ZENPROD.PRESENCE.ARCHIVER_WIFI_CONSENTED_SIGHTINGS_S3_STAGE
on_error = 'continue';
...
create stream if not exists ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE_CHANGES on table ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE;
...
create or replace task ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_UPSERT_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE_CHANGES')
AS
    MERGE INTO ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS as wfs USING (
        SELECT
            ...
            ZENPROD.PRESENCE.DECODE_CLIENT_MAC_INFO($1:client_mac_info::array) as anonymous_client_mac_info,
        ...
        FROM ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_ARCHIVE_CHANGES QUALIFY ROW_NUMBER() OVER (PARTITION BY sighting_id order by INSERT_ID desc) = 1
...
ALTER TASK ZENPROD.PRESENCE.WIFI_CONSENTED_SIGHTINGS_UPSERT_TASK RESUME;
