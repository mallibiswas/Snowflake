...
create stage if not exists ZENPROD.PRESENCE.ARCHIVER_LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_S3_STAGE
    file_format = ZENPROD.PRESENCE.PRESENCE_CSV_FORMAT
    url = 's3://zp-uw2-data-archives/rds/wifisightingsclassifierconfig/location_classifier_config_thresholds'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-rds-location-classifier-config-thresholds-snowflake-stage');

create or replace TABLE ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS (
...
create or replace TABLE ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE (
...
create pipe if not exists ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_SNOWPIPE auto_ingest=true as
COPY INTO ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE (
	LOCATION_ID,
...
FROM @ZENPROD.PRESENCE.ARCHIVER_LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_S3_STAGE
on_error = 'continue';

create stream if not exists ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES on table ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE;

create task if not exists ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_UPSERT_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES')
AS
    MERGE INTO ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS as lcgt USING (
        SELECT
            location_id,
            min_walkin_dwell_time_seconds,
            max_walkin_dwell_time_seconds,
        ...
        FROM ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES
...
ALTER TASK ZENPROD.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS_UPSERT_TASK RESUME;
