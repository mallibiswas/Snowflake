...
create or replace procedure ZENPROD.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENPROD.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM as
          select
            $1 as id,
          ...
          FROM @ZENPROD.AMS_ACCOUNTS.AMS_ACCOUNTS_S3_STAGE/${FILE_DATE}/salesforce_quote_line_item.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENPROD.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENPROD.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_PROCEDURE(CURRENT_DATE());

alter task ZENPROD.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_TASK resume;