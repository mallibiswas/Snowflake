...
create or replace procedure ZENPROD.AMS.CONTRACT_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENPROD.AMS.CONTRACT as
          select
            $1 as id,
          ...
          FROM @ZENPROD.AMS.AMS_S3_STAGE/${FILE_DATE}/contract.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENPROD.AMS.CONTRACT_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENPROD.AMS.CONTRACT_PROCEDURE(CURRENT_DATE());

alter task ZENPROD.AMS.CONTRACT_TASK resume;