...
create or replace procedure ZENPROD.AMS.SUBSCRIPTIONS_V2_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENPROD.AMS.SUBSCRIPTIONS_V2 as
          select
            $1::number as id,
          ...
          FROM @ZENPROD.AMS.AMS_S3_STAGE/${FILE_DATE}/subscriptions_v2.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENPROD.AMS.SUBSCRIPTIONS_V2_TASK
    WAREHOUSE = ZENPROD
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENPROD.AMS.SUBSCRIPTIONS_V2_PROCEDURE(CURRENT_DATE());

alter task ZENPROD.AMS.SUBSCRIPTIONS_V2_TASK resume;