...
create OR replace stage ZENPROD.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zp-uw2-foundation-kafka-archives/archiver/presence_wifi_unknownap_blip_byclientmac/'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-archiver-wifi-unknownap-blips-snowflake-stage');

-- Table will hold archiver data from our snowpipe
create table if not exists ZENPROD.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS (
...
create pipe if not exists ZENPROD.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE auto_ingest = true as 
COPY INTO ZENPROD.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS
FROM (SELECT 
