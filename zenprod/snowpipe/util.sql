...
create function if not exists ZENPROD.PRESENCE.string_to_mac(A string)
    returns string
    language javascript
  as
$$
...
create or replace function ZENPROD.PRESENCE.decode_client_mac_info(A array)
    returns array
    language javascript
  as
$$
...
create or replace file format ZENPROD.PRESENCE.S3_PARQUET_FORMAT
  type = 'PARQUET'
