...
create OR replace stage ZENPROD.PRESENCE.CONSENTED_SIGHTINGS_S3_STAGE
    ...
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-consentedsightings-snowflake-stage');

create or replace table ZENPROD.PRESENCE.CONSENTED_SIGHTINGS (
...
create or replace pipe ZENPROD.PRESENCE.CONSENTED_SIGHTINGS_SNOWPIPE auto_ingest=true as
COPY INTO ZENPROD.PRESENCE.CONSENTED_SIGHTINGS
FROM (SELECT ZENPROD.PRESENCE.string_to_mac(lower($1:client_mac::string)) as client_mac,
    $1:contact:id::string as contact_id,
...
FROM @ZENPROD.PRESENCE.CONSENTED_SIGHTINGS_S3_STAGE)
on_error = 'continue';
