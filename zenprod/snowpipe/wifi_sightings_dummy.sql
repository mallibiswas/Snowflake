-------------------------------------------------------------------
----------------- Wifi Sightings DUMMY Snowpipe Used for testing wifisightingsv2 vs old sightings correctness
-------------------------------------------------------------------

create OR replace stage ZENPROD.PRESENCE.WIFI_SIGHTING_DUMMY_S3_STAGE
    file_format = ( format_name = 'ZENPROD.PRESENCE.S3_PARQUET_FORMAT' )
    url = 's3://zp-uw2-foundation-kafka-archives/secor/presence_wifi_sighting_bycontactidaccountid_dummy/'
    credentials = ( aws_role = 'arn:aws:iam::918769896082:role/zp-uw2-wifi-sighting-dummy-snowflake-stage');

create or replace table ZENPROD.PRESENCE.WIFI_SIGHTING_DUMMY (
...
create or replace pipe ZENPROD.PRESENCE.WIFI_SIGHTING_DUMMY_SNOWPIPE auto_ingest=true as
COPY INTO ZENPROD.PRESENCE.WIFI_SIGHTING_DUMMY
FROM (SELECT 
...
FROM @ZENPROD.PRESENCE.WIFI_SIGHTING_DUMMY_S3_STAGE)
on_error = 'continue';
