#!bin/bash
SQLDIR=$HOME/snowflake/zenalytics/ams_v3/routers
SETUPDIR=$HOME/snowflake/zenalytics/ams_v3/initialize
LOGDIR=$HOME/snowflake/zenalytics/ams_v3/routers/log
#
file_ext=csv
logfile_ext=$(date +%Y%m%d_%H:%M:%S)
logfile=AMS_routers_load_daily_data_$logfile_ext.log
stagename=ZENALYTICS.public.s3_ams_routers_stage
dbname=ZENALYTICS
whname=ZENLOADER
rolename=ETL_PROD_ROLE
schemaname=AMS_ROUTERS
stageschemaname=_STAGING
#
# get latest load date
load_date=$(aws s3 ls s3://zp-uw2-data-archives/rds/ams-routers/ | sort | tail -n 1 | awk '{print $2}'| cut -d'/' -f 1)
#
echo load date = $load_date
#
stagepath=$load_date
stageurl=s3://zp-uw2-data-archives/rds/ams-routers
