...
create or replace stage s3_enrichedsightings_stage
  file_format = s3_parquet_format
  url = 's3://zp-uw2-foundation-kafka-archives/secor/enrichedsightings_enrichedsighting_byclientmac_0/'
  credentials = (aws_key_id='******************' aws_secret_key='***************************');


create or replace pipe ZENALYTICS._STAGING.enriched_sightings_snowpipe auto_ingest=true as
COPY INTO ZENALYTICS._STAGING._enrichedsightings_enrichedsighting_byclientmac_0_
(jsontext, insert_dttm)
FROM
(
SELECT  $1::variant as jsontext,
        current_timestamp() as insert_dttm        
FROM @s3_enrichedsightings_stage
)
on_error = 'continue'
;
******/
...
create or replace pipe ZENALYTICS._STAGING.enriched_sightings_snowpipe auto_ingest=true as
COPY INTO ZENALYTICS.PRESENCE.ENRICHED_SIGHTINGS
FROM
(
