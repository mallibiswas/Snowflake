CREATE OR REPLACE VIEW CAMPAIGN_WALKTHROUGHS COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for camapaign-level location walkthroughs reported daily'
AS
...
, walkthroughs_until_2019_11_15_ as (
  select 
    ...
    , cd.insight_type
    , cd.date
    , ifnull(i.walkthroughs,0) as confirmed_walkthroughs
    , s.sample_rate_multiplier
    , case when cg.campaign_goal = 'CUSTOMER_ACQUISITION' then round(i.walkthroughs * s.sample_rate_multiplier)
         when cg.campaign_goal = 'LOYALTY' then round(i.walkthroughs * 1) -- no sampling required
         when cg.campaign_goal = 'LOST_CUSTOMER' then round(i.walkthroughs * 1) -- no sampling required
         when cg.campaign_goal is null then round(i.walkthroughs * s.sample_rate_multiplier) -- if unknown, default to sampling
  ...
  left join zenalytics.ads_campaigns.campaigns c on cd.campaign_id = c.campaign_id
...
, ad_set_walkthroughs_after_2019_11_15_ as (
  select 
    ...
    , sum(i.walkthroughs) as confirmed_walkthroughs
    , sum(case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(i.walkthroughs * s.sample_rate_multiplier)
  ...
  from campaign_dates_ cd
  left join zenalytics.ads_campaigns.ad_insights i 
    on cd.ad_account_id = i.ad_account_id
    and cd.campaign_id = i.campaign_id
    and cd.insight_type = i.insight_type
    and cd.date = try_to_date(i.breakdown_value)
...
, walkthroughs_after_2019_11_15_ as (
  ...
  left join zenalytics.ads_campaigns.campaigns c on w.campaign_id = c.campaign_id
...
, custom_conversion_ad_insights_ as (
  ...
  from zenalytics.ads_campaigns.ad_insights_custom_conversions aiicc
  left join zenalytics.ads_campaigns.ads a on aiicc.ad_id = a.ad_id
...
, custom_conversion_walkthroughs_ as (
  ...
  left join zenalytics.ads_campaigns.sample_rates s on cccd.business_id = s.business_id and cccd.date = s.day 
  group by 1,2,3,4,5
)
...
, campaign_walkthroughs_ as (
  ...
  left join zenalytics.ads_campaigns.campaigns c on ccw.campaign_id = c.campaign_id
)
