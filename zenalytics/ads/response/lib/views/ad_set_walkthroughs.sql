CREATE OR REPLACE VIEW AD_SET_WALKTHROUGHS COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for ad set level metrics such as impressions, clicks & spend reported daily'
AS
...
, custom_conversion_campaigns_ as (
  ...
  from zenalytics.ads_campaigns.insights_custom_conversions icc
  left join zenalytics.ads_campaigns.campaigns c on icc.campaign_id = c.campaign_id
  left join zenalytics.ads_campaigns.zenreach_campaign_records zcr on zcr.campaign_id = c.campaign_id
  where breakdown_type = 'DAY'
  and zcr.zenreach_campaign_id in (select zenreach_campaign_id from zenalytics.ads_campaigns.zenreach_ad_account_campaigns)
  group by 1,2,3
)
...
, ad_set_walkthroughs_after_2019_11_15_ as (
  select 
    ...
    , case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(i.walkthroughs * s.sample_rate_multiplier)
           when ad_set_goal = 'LOYALTY' then round(i.walkthroughs * 1) -- no sampling required
           when ad_set_goal = 'LOST_CUSTOMER' then round(i.walkthroughs * 1) -- no sampling required
           when ad_set_goal is null then round(i.walkthroughs * s.sample_rate_multiplier) -- if unknown, default to sampling
           else round(i.walkthroughs * s.sample_rate_multiplier)
         end as total_walkthroughs
    , i.insight_id as ad_set_insight_ids
  from ad_set_dates_ cd
  ...
  left join zenalytics.ads_campaigns.zenreach_ad_set_goals zasg on i.ad_set_id = zasg.ad_set_id
  -- left join zenalytics.ads_campaigns.sample_rates s on coalesce(a.business_id, zcrl.location_id) = s.business_id and cd.date = s.day
  left join zenalytics.ads_campaigns.sample_rates s on zcrl.location_id = s.business_id and cd.date = s.day
  left join zenalytics.ads_campaigns.campaigns c on i.campaign_id = c.campaign_id
  ...
  where ac.is_zenreach = TRUE
    ...
    and date <= dateadd(days, 8, max_date_with_spend) 
...
, custom_conversion_locations_ as (
   ...
   from zenalytics.ads_campaigns.offline_event_set_conversions oesc
   left join zenalytics.ads_campaigns.custom_conversions cc on cc.custom_conversion_id = oesc.custom_conversion_id
)
...
, custom_conversion_ad_set_dates_ as (
  select *
  from all_dates_ d
...
, custom_conversion_ad_set_insights_ as (
  select 
      ...
      , coalesce(zasg.goal,cg.campaign_goal) as ad_set_goal
      , aiicc.walkthroughs
      , aiicc.insights_ad_sets_custom_id
  from zenalytics.ads_campaigns.ad_set_insights_custom_conversions aiicc
  left join zenalytics.ads_campaigns.ad_sets a on aiicc.ad_set_id = a.ad_set_id
  left join zenalytics.ads_campaigns.zenreach_ad_set_goals zasg on a.ad_set_id = zasg.ad_set_id
...
, custom_conversion_walkthroughs_ as (
  select 
    ...
    , ccad.insight_type
    , ccad.date
    , ifnull(ccai.walkthroughs,0) as confirmed_walkthroughs
    , s.sample_rate_multiplier as sample_rate_multiplier
    , case when ad_set_goal = 'CUSTOMER_ACQUISITION' then round(ifnull(ccai.walkthroughs,0) * s.sample_rate_multiplier)
         when ad_set_goal = 'LOYALTY' then round(ifnull(ccai.walkthroughs,0) * 1) -- no sampling required
         when ad_set_goal = 'LOST_CUSTOMER' then round(ifnull(ccai.walkthroughs,0) * 1) -- no sampling required
         when ad_set_goal is null then round(ifnull(ccai.walkthroughs,0) * s.sample_rate_multiplier) -- if unknown, default to sampling
         else round(ifnull(ccai.walkthroughs,0) * s.sample_rate_multiplier)
      end as total_walkthroughs
  from custom_conversion_ad_set_dates_ ccad
  left join custom_conversion_ad_set_insights_ ccai
  ...
  left join zenalytics.ads_campaigns.sample_rates s on ccad.business_id = s.business_id and ccad.date = s.day 
)
select ad_account_id, campaign_id, ad_set_id, business_id, insight_type, date, confirmed_walkthroughs, sample_rate_multiplier, total_walkthroughs
from ad_set_walkthroughs_after_2019_11_15_;
