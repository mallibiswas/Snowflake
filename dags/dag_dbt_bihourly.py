...
with DAG(dag_id='dbt_bihourly', max_active_runs=1,  catchup=False, default_args=default_args, schedule_interval='00 2-22/2 * * *') as dag:
    ...
    dbt_run = BashOperator(
        task_id='dbt_run',
        env=env,
        bash_command='/usr/local/airflow/.local/bin/dbt run --profiles-dir /usr/local/airflow/dags/dbt_analytics --project-dir /usr/local/airflow/dags/dbt_analytics --models sources.ams staging.ams'
    )

    dbt_test = BashOperator(
        task_id='dbt_test',
        env=env,
        bash_command='/usr/local/airflow/.local/bin/dbt test --profiles-dir /usr/local/airflow/dags/dbt_analytics --project-dir /usr/local/airflow/dags/dbt_analytics --models sources.ams staging.ams'
    )
