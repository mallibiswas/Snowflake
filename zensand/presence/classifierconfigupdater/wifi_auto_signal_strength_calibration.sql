...
create table if not exists ZENSAND.PRESENCE.WIFI_AUTO_SIGNAL_STRENGTH_CALIBRATION (
...
create or replace task ZENSAND.PRESENCE.WIFI_CALCULATE_SIGNAL_STRENGTH_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON 0 0 * * 1 America/Los_Angeles' -- At 00:00 on Monday in PDT tz
AS
    MERGE INTO ZENSAND.PRESENCE.WIFI_AUTO_SIGNAL_STRENGTH_CALIBRATION as calibration USING (
        ...
        , previous_config_ AS (
            ...
            FROM ZENSAND.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS l
            JOIN (
                SELECT
                    location_id
                    , MAX(effective_as_of) AS last_set
                FROM ZENSAND.PRESENCE.LOCATION_CLASSIFIER_CONFIG_THRESHOLDS
                GROUP BY location_id
        ...
        , calibration_bundle_ AS (
            ...
            JOIN ZENSAND.CRM.PORTAL_BUSINESSPROFILE bp ON s.location_id = bp.business_id
        )
...
ALTER TASK IF EXISTS  ZENSAND.PRESENCE.WIFI_CALCULATE_SIGNAL_STRENGTH_TASK RESUME;
