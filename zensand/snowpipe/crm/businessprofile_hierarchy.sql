CREATE OR REPLACE view ZENSAND.CRM.BUSINESSPROFILE_HIERARCHY
...
from zensand.crm.portal_businessprofile where parent_id is null) Q1
LEFT OUTER JOIN
...
where parent_id in (select business_id from zensand.crm.portal_businessprofile where parent_id is null)
...
where parent_id in (select business_id
                    from zensand.crm.portal_businessprofile
                    where parent_id in
                    (select business_id from zensand.crm.portal_businessprofile where parent_id is null))
...
from zensand.crm.portal_businessprofile
where parent_id in (
select business_id from zensand.crm.portal_businessprofile
where parent_id in (select business_id
                    from zensand.crm.portal_businessprofile
                    where parent_id in
                          (select business_id from zensand.crm.portal_businessprofile where parent_id is null))
