...
create or replace procedure ZENSAND.CRM.PORTAL_ROUTER_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSAND.CRM.PORTAL_ROUTER as
    select  
    ...
    FROM @ZENSAND.CRM.MONGO_S3_STAGE/${FILE_DATE}/portal_router.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSAND.CRM.PORTAL_ROUTER_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSAND.CRM.PORTAL_ROUTER_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.CRM.PORTAL_ROUTER_TASK resume;
