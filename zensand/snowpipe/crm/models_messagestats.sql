...
create or replace procedure ZENSAND.CRM.MODELS_MESSAGESTATS_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSAND.CRM.MODELS_MESSAGESTATS as
    select  
    ...
    FROM @ZENSAND.CRM.MONGO_S3_STAGE/${FILE_DATE}/models_messagestats.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSAND.CRM.MODELS_MESSAGESTATS_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSAND.CRM.MODELS_MESSAGESTATS_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.CRM.MODELS_MESSAGESTATS_TASK resume;
