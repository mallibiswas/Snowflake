...
create or replace procedure ZENSAND.CRM.ANALYTICS_TRAFFIC_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSAND.CRM.ANALYTICS_TRAFFIC as
    select  
    ...
    FROM @ZENSAND.CRM.MONGO_S3_STAGE/${FILE_DATE}/analytics_traffic.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSAND.CRM.ANALYTICS_TRAFFIC_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSAND.CRM.ANALYTICS_TRAFFIC_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.CRM.ANALYTICS_TRAFFIC_TASK resume;
