...
create or replace procedure ZENSAND.CRM.SMBSITE_EMAILIMPORT_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
  sqlText:`
    create or replace transient table ZENSAND.CRM.SMBSITE_EMAILIMPORT as
    select  
    ...
    FROM @ZENSAND.CRM.MONGO_S3_STAGE/${FILE_DATE}/smbsite_emailimport.json;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute) 
create task ZENSAND.CRM.SMBSITE_EMAILIMPORT_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSAND.CRM.SMBSITE_EMAILIMPORT_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.CRM.SMBSITE_EMAILIMPORT_TASK resume;
