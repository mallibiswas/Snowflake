-------------------------------------------------------------------
----------------- Unknown Ap Blips Snowpipe Using The New Archiver
-------------------------------------------------------------------

create OR replace stage ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/presence_wifi_unknownap_blip_byclientmac/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-wifi-unknownap-blips-snowflake-stage');

-- Table will hold archiver data from our snowpipe
create table if not exists ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS (
...
create pipe if not exists ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE auto_ingest = true as 
COPY INTO ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS
FROM (SELECT 
    nvl($1:ap_type, 'APType_UNKNOWN')::string as ap_type,
    HEX_ENCODE(BASE64_DECODE_BINARY($1:ap_mac::string)) as ap_mac,

    HEX_ENCODE(BASE64_DECODE_BINARY($1:client_mac::string)) as client_mac,
