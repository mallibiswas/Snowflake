...
create or replace procedure ZENSAND.ADS.AD_SET_INSIGHTS_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSAND.ADS.AD_SET_INSIGHTS as
          select
            $1 as insight_id,
            $2 as ad_account_id,
            $3 as campaign_id,
          ...
          FROM @ZENSAND.ADS.ARCHIVER_ADS_S3_STAGE/nwfbcampaignsync/${FILE_DATE}/insights_ad_sets_v3.csv;`
     }).execute();
$$;

-- Create task to call the procedure (at every 45th minute)
create or replace task ZENSAND.ADS.AD_SET_INSIGHTS_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENSAND.ADS.AD_SET_INSIGHTS_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.ADS.AD_SET_INSIGHTS_TASK resume;
