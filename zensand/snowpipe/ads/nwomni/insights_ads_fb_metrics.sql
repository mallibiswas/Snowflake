...
create or replace procedure ZENSAND.ADS.INSIGHTS_ADS_FB_METRICS_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
...
snowflake.createStatement({
	sqlText: `
	create or replace transient table ZENSAND.ADS.INSIGHTS_ADS_FB_METRICS as
		select
			$1 as id,
		...
		FROM @ZENSAND.ADS.ARCHIVER_ADS_S3_STAGE/nwomni/${FILE_DATE}/insights_ads_fb_metrics.csv;`
}).execute();
$$;
-- Create task to call the procedure (at every 45th minute)
create or replace task ZENSAND.ADS.INSIGHTS_ADS_FB_METRICS_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as
    CALL ZENSAND.ADS.INSIGHTS_ADS_FB_METRICS_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.ADS.INSIGHTS_ADS_FB_METRICS_TASK resume;
