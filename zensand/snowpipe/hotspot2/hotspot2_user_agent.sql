...
create or replace procedure ZENSAND.HOTSPOT2.USER_AGENT_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSAND.HOTSPOT2.USER_AGENT as
          select
            $1::string as id,
	        $2::string as os,
          ...
          FROM @ZENSAND.HOTSPOT2.HOTSPOT2_S3_STAGE/${FILE_DATE}/user_agent.csv;`
     }).execute();
$$;


-- Create task to call the procedure (need to avoid overlapping the S3 archiver, S3 is 0, 12 hour)
create or replace task ZENSAND.HOTSPOT2.USER_AGENT_TASK
    WAREHOUSE = ZENSAND
...
as 
    ...
    CALL ZENSAND.HOTSPOT2.USER_AGENT_PROCEDURE(TO_DATE(CONVERT_TIMEZONE('UTC', CURRENT_TIMESTAMP())));

alter task ZENSAND.HOTSPOT2.USER_AGENT_TASK resume;
