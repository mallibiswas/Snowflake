...
create or replace procedure ZENSAND.POS.MERCHANT_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSAND.POS.MERCHANT as	
          select
            $1::string as id,
          ...
          FROM @ZENSAND.POS.ARCHIVER_POS_S3_STAGE/merchant/${FILE_DATE}.csv;`
     }).execute();
$$;

-- Create task to call the procedure
create task ZENSAND.POS.MERCHANT_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON 0 20 * * * UTC'
as 
    CALL ZENSAND.POS.MERCHANT_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.POS.MERCHANT_TASK resume;
