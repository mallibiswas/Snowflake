...
create or replace procedure ZENSAND.AMS.CONTRACT_V2_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSAND.AMS.CONTRACT_V2 as
          select
            $1 as id,
          ...
          FROM @ZENSAND.AMS.AMS_S3_STAGE/${FILE_DATE}/contract_v2.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSAND.AMS.CONTRACT_V2_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENSAND.AMS.CONTRACT_V2_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.AMS.CONTRACT_V2_TASK resume;