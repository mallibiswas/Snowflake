...
create or replace procedure ZENSAND.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_PROCEDURE(FILE_DATE STRING)
returns boolean
language javascript
strict
as
$$
    snowflake.createStatement({ 
      sqlText: `
          create or replace transient table ZENSAND.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM as
          select
            $1 as id,
          ...
          FROM @ZENSAND.AMS_ACCOUNTS.AMS_ACCOUNTS_S3_STAGE/${FILE_DATE}/salesforce_quote_line_item.csv;`
     }).execute();
$$;


-- Create task to call the procedure (at every 45th minute)
create task ZENSAND.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON */45 * * * * UTC'
as 
    CALL ZENSAND.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_PROCEDURE(CURRENT_DATE());

alter task ZENSAND.AMS_ACCOUNTS.SALESFORCE_QUOTE_LINE_ITEM_TASK resume;