...
create OR replace stage ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_BLIPS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/presence_wifi_enrichedblip_bylocationid/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-wifi-enrichedblips-snowflake-stage');

-- Table will hold archiver data from our snowpipe
create table if not exists ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_BLIPS (
...
create pipe if not exists ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_BLIPS_SNOWPIPE auto_ingest = true as 
COPY INTO ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_BLIPS
FROM (SELECT 
...
FROM @ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_BLIPS_S3_STAGE)
on_error = 'continue';
