create stage if not exists ZENSAND.PRESENCE.ARCHIVER_WIFI_FINISHED_SIGHTINGS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/presence_wifi_finishedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-wifi-finishedsighting-snowflake-stage');

create table if not exists ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS (
...
create table if not exists ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_ARCHIVE (
...
create pipe if not exists ZENSAND.PRESENCE.ARCHIVER_WIFI_FINISHED_SIGHTINGS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_ARCHIVE(RAW_RECORD)
FROM @ZENSAND.PRESENCE.ARCHIVER_WIFI_FINISHED_SIGHTINGS_S3_STAGE
on_error = 'continue';
...
create stream if not exists ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_ARCHIVE_CHANGES on table ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_ARCHIVE;
...
create or replace task ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_UPSERT_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_ARCHIVE_CHANGES')
AS
    MERGE INTO ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS as wfs USING (
        SELECT
            ...
            ZENSAND.PRESENCE.DECODE_CLIENT_MAC_INFO($1:client_mac_info::array) as client_mac_info,
        ...
        FROM ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_ARCHIVE_CHANGES QUALIFY ROW_NUMBER() OVER (PARTITION BY sighting_id order by INSERT_ID desc) = 1
...
ALTER TASK ZENSAND.PRESENCE.WIFI_FINISHED_SIGHTINGS_UPSERT_TASK RESUME;