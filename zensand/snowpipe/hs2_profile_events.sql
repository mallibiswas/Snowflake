...
create OR replace stage  ZENSAND.PRESENCE.ARCHIVER_HS2_PROFILEEVENTS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/hs2_profileevents_byprofileid/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-hs2-profileevents-snowflake-stage');

create or replace table ZENSAND.PRESENCE.ARCHIVER_HS2_PROFILEEVENTS (
...
create or replace pipe ZENSAND.PRESENCE.ARCHIVER_HS2_PROFILEEVENTS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSAND.PRESENCE.ARCHIVER_HS2_PROFILEEVENTS
FROM (SELECT 
     ...
     FROM @ZENSAND.PRESENCE.ARCHIVER_HS2_PROFILEEVENTS_S3_STAGE)
on_error = 'continue';
