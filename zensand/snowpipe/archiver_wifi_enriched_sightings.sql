...
create OR replace stage ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/presence_wifi_enrichedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-wifi-enrichedsighting-snowflake-stage');

create or replace table ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS (
...
COPY INTO ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS
FROM (SELECT 
