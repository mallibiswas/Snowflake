...
-- pause pipe
alter pipe ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_SNOWPIPE set pipe_execution_paused = true;
-- ensure pending files is 0
select system$pipe_status('ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_SNOWPIPE'); 
-- re-create pipe to copy into the old table
create or replace pipe ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSAND.PRESENCE.WIFI_ENRICHED_SIGHTINGS
FROM (SELECT 
...
FROM @ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_S3_STAGE)
on_error = 'continue';
-- ensure pipe execution state is running
select system$pipe_status('ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS_SNOWPIPE'); 

-- remove unnecessary new table
drop table if exists ZENSAND.PRESENCE.ARCHIVER_WIFI_ENRICHED_SIGHTINGS


-- Update ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE

-- pause pipe
alter pipe ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE set pipe_execution_paused = true;
-- ensure pending files is 0
select system$pipe_status('ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE'); 
-- re-create pipe to copy into the old table
create or replace pipe ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSAND.PRESENCE.WIFI_UNKNOWNAP_BLIP
FROM (SELECT 
    nvl($1:ap_type, 'APType_UNKNOWN')::string as ap_type,
    ZENSAND.PRESENCE.string_to_mac(HEX_ENCODE(BASE64_DECODE_BINARY($1:ap_mac::string))) as ap_mac,

    ZENSAND.PRESENCE.string_to_mac(HEX_ENCODE(BASE64_DECODE_BINARY($1:client_mac::string))) as client_mac,
...
FROM @ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_S3_STAGE)
on_error = 'continue';
-- ensure pipe execution state is running
select system$pipe_status('ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS_SNOWPIPE'); 

-- remove unnecessary new table
drop table if exists ZENSAND.PRESENCE.ARCHIVER_WIFI_UNKNOWN_AP_BLIPS
