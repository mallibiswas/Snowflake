...
create OR replace stage ZENSAND.PRESENCE.WIFI_ENRICHED_SIGHTINGS_S3_STAGE
    file_format = ( format_name = 'ZENSAND.PRESENCE.S3_PARQUET_FORMAT' )
    url = 's3://zd-uw2-platform-kafka-archives/secor/presence_wifi_enrichedsighting_bycontactidlocationid/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-wifi-enrichedsighting-snowflake-stage');

create or replace table ZENSAND.PRESENCE.WIFI_ENRICHED_SIGHTINGS (
...
create or replace pipe ZENSAND.PRESENCE.WIFI_ENRICHED_SIGHTINGS_SNOWPIPE auto_ingest=true as
...
FROM @ZENSAND.PRESENCE.WIFI_ENRICHED_SIGHTINGS_S3_STAGE)
on_error = 'continue';
