...
create stage if not exists ZENSAND.PRESENCE.ARCHIVER_GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_S3_STAGE
    file_format = ZENSAND.PRESENCE.PRESENCE_CSV_FORMAT
    url = 's3://zd-uw2-data-archives/rds/wifisightingsclassifierconfig/global_classifier_config_thresholds'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-rds-global-classifier-config-thresholds-snowflake-stage');

create or replace TABLE ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS (
...
create table if not exists ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE (
...
create pipe if not exists ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE (
	LOCK,
...
FROM @ZENSAND.PRESENCE.ARCHIVER_GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_S3_STAGE
on_error = 'continue';

create stream if not exists ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES on table ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_ARCHIVE;

create task if not exists ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES')
AS
    UPDATE ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS
        SET
        ...
        FROM ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_CHANGES as global_changes
    ;

ALTER TASK ZENSAND.PRESENCE.GLOBAL_CLASSIFIER_CONFIG_THRESHOLDS_TASK RESUME;
