...
create OR replace stage ZENSAND.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/anonymizedblips_anonymizedblip_byclientmac_0/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-anonymizedblips-snowflake-stage');

-- Table will hold archiver data from our snowpipe
create table if not exists ZENSAND.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS (
...
create pipe if not exists ZENSAND.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS_SNOWPIPE auto_ingest = true as 
COPY INTO ZENSAND.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS
FROM (SELECT 
...
FROM @ZENSAND.PRESENCE.ARCHIVER_ANONYMIZED_BLIPS_S3_STAGE)
on_error = 'continue';
