...
CREATE OR REPLACE STAGE ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_S3_STAGE
    file_format = ( TYPE = JSON )
    url = 's3://zd-uw2-platform-kafka-archives/archiver/mail_email_walkthroughs_bymessagelogid/'
    credentials = ( aws_role = 'arn:aws:iam::504740723475:role/zd-uw2-archiver-mail-walkthroughs-snowflake-stage');


CREATE OR REPLACE TABLE ZENSAND.PRESENCE.MAIL_WALKTHROUGHS(
...
CREATE OR REPLACE TABLE ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE(
...
CREATE PIPE IF NOT EXISTS ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_SNOWPIPE auto_ingest=true as
COPY INTO ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE(RAW_RECORD)
FROM @ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_S3_STAGE
on_error = 'continue';
...
CREATE STREAM IF NOT EXISTS ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE_CHANGES ON TABLE ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE;
...
create task if not exists ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_UPSERT_TASK
    WAREHOUSE = ZENSAND
    SCHEDULE = 'USING CRON * * * * * UTC'
WHEN
    SYSTEM$STREAM_HAS_DATA('ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE_CHANGES')
AS
    MERGE INTO ZENSAND.PRESENCE.MAIL_WALKTHROUGHS as wt USING (
        SELECT
        ...
        FROM ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_ARCHIVE_CHANGES QUALIFY ROW_NUMBER() OVER (PARTITION BY messagelog_id order by INSERT_ID desc) = 1
...
ALTER TASK ZENSAND.PRESENCE.MAIL_WALKTHROUGHS_UPSERT_TASK RESUME;
