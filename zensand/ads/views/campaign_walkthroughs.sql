CREATE OR REPLACE VIEW ZENSAND.ADS.CAMPAIGN_WALKTHROUGHS COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for camapaign-level location walkthroughs reported daily'
AS
...
, walkthroughs_until_2019_11_15_ as (
  ...
  left join zensand.ads.insights i 
  ...
  left join zensand.ads.campaigns c on cd.campaign_id = c.campaign_id
...
, ad_set_walkthroughs_after_2019_11_15_ as (
  ...
  from campaign_dates_ cd
  left join zensand.ads.ad_insights i 
    on cd.ad_account_id = i.ad_account_id
    and cd.campaign_id = i.campaign_id
    and cd.insight_type = i.insight_type
    and cd.date = try_to_date(i.breakdown_value)
...
, walkthroughs_after_2019_11_15_ as (
  ...
  left join zensand.ads.campaigns c on w.campaign_id = c.campaign_id
  group by 1,2,3,4,5,6,7,8
)
...
, custom_conversion_ad_insights_ as (
  ...
  from zensand.ads.ad_insights_custom_conversions aiicc
  left join zensand.ads.ads a on aiicc.ad_id = a.ad_id
...
, custom_conversion_walkthroughs_ as (
  ...
  left join zensand.ads.sample_rates s on cccd.business_id = s.business_id and cccd.date = s.day 
  group by 1,2,3,4,5
)
...
, campaign_walkthroughs_ as (
  ...
  left join zensand.ads.campaigns c on ccw.campaign_id = c.campaign_id
)
