CREATE OR REPLACE VIEW ZENSAND.ADS.AD_SET_WALKTHROUGHS COMMENT='IN DEVELOPMENT 2020-07-16 - unified ads reporting view for ad set level metrics such as impressions, clicks & spend reported daily'
AS
...
, custom_conversion_campaigns_ as (
  ...
  from zensand.ads.insights_custom_conversions icc
  left join zensand.ads.campaigns c on icc.campaign_id = c.campaign_id
  left join zensand.ads.zenreach_campaign_records zcr on zcr.campaign_id = c.campaign_id
  where breakdown_type = 'DAY'
  and zcr.zenreach_campaign_id in (select zenreach_campaign_id from zensand.ads.zenreach_ad_account_campaigns)
  group by 1,2,3
)
...
, ad_set_date_range_ as (
  ...
  from zensand.ads.ad_set_insights i
...
, zenreach_campaign_records_locations_ as (
    ...
    from zensand.ads.zenreach_campaign_records_locations
)
, campaign_goals_ as (
  select
    zcr.campaign_id
  ...
  from zensand.ads.zenreach_campaigns zc
  left join zensand.ads.zenreach_campaign_records zcr
      on zc.zenreach_campaign_id = zcr.zenreach_campaign_id
  left join zensand.ads.zenreach_campaign_visibility zcv on
...
, ad_set_walkthroughs_after_2019_11_15_ as (
  ...
  left join zensand.ads.ad_set_insights i 
  ...
  left join zensand.ads.zenreach_campaigns zc
    on zcr.zenreach_campaign_id = zc.zenreach_campaign_id
  left join zensand.ads.zenreach_campaign_records_locations zcrl
  ...
  left join zensand.ads.zenreach_ad_set_goals zasg on i.ad_set_id = zasg.ad_set_id
  -- left join zensand.ads.sample_rates s on coalesce(a.business_id, zcrl.location_id) = s.business_id and cd.date = s.day
  left join zensand.ads.sample_rates s on zcrl.location_id = s.business_id and cd.date = s.day
  left join zensand.ads.campaigns c on i.campaign_id = c.campaign_id
  join zensand.ads.ad_accounts ac
...
, custom_conversion_locations_ as (
   ...
   from zensand.ads.offline_event_set_conversions oesc
   left join zensand.ads.custom_conversions cc on cc.custom_conversion_id = oesc.custom_conversion_id
)
...
, ad_account_campaign_locations_mapping as (
  select
      zcr.campaign_id
      , zcrl.location_id as business_id
  from zensand.ads.zenreach_campaign_records zcr
  inner join zensand.ads.zenreach_campaign_records_locations zcrl on zcr.zenreach_campaign_records_id = zcrl.zenreach_campaign_records_id
)
...
, custom_conversion_ad_set_insights_ as (
  ...
  from zensand.ads.ad_set_insights_custom_conversions aiicc
  left join zensand.ads.ad_sets a on aiicc.ad_set_id = a.ad_set_id
  left join zensand.ads.zenreach_ad_set_goals zasg on a.ad_set_id = zasg.ad_set_id
...
, custom_conversion_walkthroughs_ as (
  ...
  left join zensand.ads.sample_rates s on ccad.business_id = s.business_id and ccad.date = s.day 
)
